...
static ERL_NIF_TERM sasl_cli_new(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    int result;
    result = sasl_client_new((const char *)service.data, (const char *)host.data, NULL, NULL, state->callbacks, 0, &state->conn);
    return enif_make_int(env, result);
}
...
static ERL_NIF_TERM sasl_list_mech(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    sasl_listmech(state->conn, NULL, NULL, " ", NULL, &avail_mechs, NULL, NULL);
...
static ERL_NIF_TERM sasl_cli_step(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    result = sasl_client_step(state->conn,
            in, challenge.size,
            &interact,
            &out, &outlen);
    free(in);
...
{
    return enif_make_string(env, sasl_errdetail(state->conn), ERL_NIF_LATIN1);
}
